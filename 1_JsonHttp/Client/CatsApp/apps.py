...
class CatsApp(object):
    def __init__(self):
        ...
        self.statsUrl = "https://localhost:7264/cat_stats"
        self.currentCat = 1
        self.totalCats = -1
        super().__init__()
    ...
    def index(self, catNum=1):
        ...
        # אין צורך לבקש כל הזמן
        if self.totalCats == -1:
            # ניקח את כמות החתולים מהג'ייסון
            stats = requests.get(self.statsUrl, verify=False)
            statsObj = stats.json()

            self.totalCats = statsObj['Amount']
